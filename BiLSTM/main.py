...
def main():
    ...
    for line in train_data:
        total_tokens += len(line[0])
    

    # print the statistics of the data
    if args.mode == "train":
        print (f"Total Tokens in Training Dataset: {total_tokens}")
    ...
    headers = [key for key in sorted(params)] + ["test_loss", "test_accuracy", "trainable_parameters"]
    headers.insert(0, "language")
    row = [v for k, v in sorted(params.items())] 
    row.insert(0, args.lang)
    filename = "_".join(str(x) for x in row)
    row = row + [test_loss, test_acc, n_param]

    with open('model_evaluation/{}.csv'.format(filename), 'w') as f:
        write = csv.writer(f)
...
def dump_output(data_, outputs, vocab_tag, output_path):
    assert len(data_) == len(outputs)
    with open(output_path, 'w', encoding="utf-8") as f:
        for (line, _), output in zip(data_, outputs):
